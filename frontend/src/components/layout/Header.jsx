...
export default function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white dark:bg-stone-900 shadow-md' : 'bg-white/95 dark:bg-stone-900/95 backdrop-blur-sm'}`}>
      <div className="section">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="flex items-center gap-1.5 lg:gap-2 flex-shrink-0">
            ...
            {/* Favorites */}
            <Link to="/favorites" className={`relative p-2 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors flex-shrink-0 ${favorites.length > 0 ? 'text-red-600' : 'text-stone-600 dark:text-stone-300'}`} title={t('header.favorites')}>
            ...
            {/* Cart */}
            <Link to="/cart" className={`relative p-2 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors flex-shrink-0 ${cartCount > 0 ? 'text-orange-600' : 'text-stone-600 dark:text-stone-300'}`} title={t('header.cart')}>
            ...
            <button
              onClick={toggleDark}
              className="p-2 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors text-stone-600 dark:text-stone-300 flex-shrink-0"
            ...
            {/* Burger */}
            <button onClick={() => setOpen(!open)} className="xl:hidden p-2 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors flex-shrink-0 text-stone-600 dark:text-stone-300">
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="xl:hidden border-t border-stone-100 dark:border-stone-800 bg-white dark:bg-stone-900">
          <div className="section py-4 space-y-1">
            <form onSubmit={handleSearch} className="flex items-center bg-stone-100 dark:bg-stone-800 rounded-xl overflow-hidden mb-4">
              <input
                ...
                onChange={(e) => setSearch(e.target.value)}
                placeholder={t('header.searchPlaceholder')}
                className="bg-transparent px-4 py-2.5 text-sm outline-none flex-1 text-stone-800 dark:text-white placeholder-stone-400"
              />
              <button type="submit" className="px-3 text-stone-500 dark:text-stone-400">
            ...
            <a href={telHref} className="flex sm:hidden items-center gap-2 px-4 py-3 text-sm font-medium text-stone-600 dark:text-stone-300">
              <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
              </svg>
              {settings.phone}
